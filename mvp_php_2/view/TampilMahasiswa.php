...
class TampilMahasiswa implements KontrakView
{
	...
	function tampil()
	{
		...
		for ($i = 0; $i < $this->prosesmahasiswa->getSize(); $i++) {
			...
			<td>
				<a href='edit.php?id=" . $this->prosesmahasiswa->getId($i) . "' class='btn btn-warning'><i class='fa fa-pencil'></i> Edit</a>
				<a href='index.php?id_hapus=" . $this->prosesmahasiswa->getId($i) . "' class='btn btn-danger'><i class='fa fa-trash'></i> Hapus</a>
			</td>
			</tr>";
		}
